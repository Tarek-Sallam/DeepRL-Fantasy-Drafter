...
class DraftEnv(Env):
    def __init__(self, teams):
        ...
        self.observation_space = Dict({"roster": MultiBinary(19), "top_projections": Box(low=np.zeros(19), high=np.array(np.ones(19) * np.inf))})
        self.draftBoard = DraftBoard(teams, random.randint(1, teams), os.path.join(os.getcwd(), 'data', 'projection_data.csv'))
        self.totalPts = 0.0
        self.round = 1
        self.max_rounds = 19
        self.state = {"roster": np.zeros(19), "top_projections": np.array(self.draftBoard.getTopProjections())}
    ...
    def reset(self):
        ...
        self.state = {"roster": np.zeros(19), "top_projections": np.array(self.draftBoard.getTopProjections())} # reset the state
        return self.state
